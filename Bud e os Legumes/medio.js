...
// Configurações para o nível "medio"
const FRUIT_COUNT = 10; // 10 legumes
...
function handleCollision(fruitElement) {
    ...
    if (fruitsRemaining === 0) {
        ...
        // Mova a linha abaixo para garantir que os confetes aparecem só aqui
        celebrateWithConfetti(); 
    }
}
...
function resetGame() {
    score = 0;
    scoreDisplay.textContent = `Pontuação: ${score}`;
    createFruits(FRUIT_COUNT); // Cria exatamente 10 frutas
...
createFruits(FRUIT_COUNT); // Cria frutas no início do jogo com base no nível "medio"
